...
import { addOrgane } from "../api/personel";

const AddOrganeModal = (props) => {
  ...
  const onSubmit = async (values) => {
    let result = await addOrgane(values);
    if (!result.ok) {
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">
          {success ? <h2>Organe Ajoutée</h2> : <h2>Ajouter Organe</h2>}
        </Modal.Title>
      </Modal.Header>
